 ...
 * exception: the theme system (`@saas/theme-aurora`, approved Phase 13)
 * injects a merchant's theme colors as CSS custom properties via React's
 * inline `style={{...}}` prop. CSP nonces/hashes apply only to `<style>`
 ...
 * not guessed. Never widen either exception to a blanket `'unsafe-
 * inline'` on the element form; add a new hash only if a real violation
 * names one.
 */
